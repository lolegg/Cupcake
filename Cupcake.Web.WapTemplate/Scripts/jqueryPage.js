...
(function (window, $) {
    $.fn.pagination = function (options) {
        ...
        var count = parseInt(_dftOpts.count);
        if (count <= 0) return;
        var _ellipsis = "...";
        var _size = parseInt(_dftOpts.size) > 0 ? (parseInt(_dftOpts.size) > count ? count : parseInt(_dftOpts.size)) : 1;
        var _page = Math.ceil(count / _size);
        var _index = parseInt(_dftOpts.index) > 0 ? (parseInt(_dftOpts.index) > _page ? _page : parseInt(_dftOpts.index)) : 1;
        var _lrcount = parseInt(_dftOpts.lrCount) * 2 + 1 > _page ? parseInt((_page - 1) / 2) : parseInt(_dftOpts.lrCount);
        var _continueCount = _lrcount * 2 + 1;
        var _lCount = parseInt(_dftOpts.lCount) <= 0 ? 0 : (parseInt(_dftOpts.lCount) > _page ? _page - 1 : parseInt(_dftOpts.lCount));
        var _rCount = parseInt(_dftOpts.rCount) <= 0 ? 0 : (parseInt(_dftOpts.rCount) > _page ? _page - 1 : parseInt(_dftOpts.rCount));
